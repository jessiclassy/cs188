...
class DiscreteDistribution(dict):
    ...
    def normalize(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        normTotal = self.total()
        if normTotal != 0:
            for key in self.keys():
                self[key] = self[key] / normTotal
        return
        # raiseNotDefined()
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for ghostPos in self.allPositions:
            ...
            ghostProb = self.beliefs[ghostPos]
            self.beliefs[ghostPos] = observationProb * ghostProb
        # raiseNotDefined()
    ...
    def elapseTime(self, gameState):
        ...
        for oldPos in self.allPositions:
            ...
            for newPos in newPosDist.keys():
                tempDist[newPos] += newPosDist[newPos] * oldProb
...
class JointParticleFilter(ParticleFilter):
    ...
    def initializeUniformly(self, gameState):
        ...
        """
        self.particles = []
        possiblePermutations = list(itertools.product(self.legalPositions, repeat = self.numGhosts))
        for i in range(self.numParticles):
            particle = random.choice(possiblePermutations)
            self.particles.append(particle)
