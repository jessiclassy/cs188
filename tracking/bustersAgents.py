...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        for ghostPos in ghostPositions:
            tempDist = self.distancer.getDistance(pacmanPosition, ghostPos)
            distances[ghostPos] = tempDist
        closestGhost = min(distances, key=distances.get)
        maxDistance = dict()
        for action in legal:
            successorPosition = Actions.getSuccessor(pacmanPosition, action)
            tempDist = self.distancer.getDistance(successorPosition, closestGhost)
            maxDistance[action] = tempDist
        return min(maxDistance, key=maxDistance.get)
